...
metacog-agent -p 'Inspect this project and explain how to run its tests.'
```

Or run from source at the repository root:

```sh
go run ./cmd/metacog-agent -p 'Inspect this project and explain how to run its tests.'
```

Choose a workspace and save the output:

```sh
metacog-agent -workspace ./my-project -p 'Summarize this project.' > run.jsonl
```

You can also pass a JSON request as an argument or through stdin:

```sh
metacog-agent '{"prompt":"Summarize this project."}'
metacog-agent < request.json
```
...
Run `metacog-agent -h` for options and the JSON request fields.

## Docker

The `unrea1labs/metacog-agent` image supports Linux on AMD64 and ARM64. Run it
with a project mounted as the workspace:

```sh
docker run --rm -i --user "$(id -u):$(id -g)" \
  -e OPENAI_API_KEY -v "$PWD:/workspace" \
  unrea1labs/metacog-agent:latest -p 'Summarize this project.'
```
