		...
		for _, maxAttempts := range []int{1, 2} {
			t.Run(provider.Name+"/"+strconv.Itoa(maxAttempts), func(t *testing.T) {
				...
				code := RunMain(ctx,
					...
					io.Discard, io.Discard, Config{Name: "heuristic", ParseRequest: parseTestRequest, Providers: DefaultProviders()})
...
func TestRunnerCodexUsesSubscriptionWithoutAPIKey(t *testing.T) {
	...
	}, func() []string { return nil }, strings.NewReader(`{"prompt":"hello","model":"gpt-test","system_prompt":"my system prompt"}`), &output, &stderr, Config{Name: "heuristic", ParseRequest: parseTestRequest, Providers: DefaultProviders()})
