...
- Forked Unreal Agent as Heuristic; module `github.com/ItIsCuthNotCup/heuristic`, binary `heuristic`. Upstream `UNREAL_HARNESS_*` env vars still work; `HEURISTIC_*` equivalents win.
...
- `heuristic` runner: `HEURISTIC_*` env config, `"metacog"` request object overrides, `-metacog` flag, trace events as `metacog {...}` JSON lines on stderr.
